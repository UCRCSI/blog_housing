...
housing_weighted %>% group_by(HOUSEHOLDRACE) %>% summarize(n = survey_total()) -> race_household
housing_weighted %>% group_by(HHRACE3) %>% summarize(n = survey_total()) -> race_HH
...
# write.xlsx(tmp, "exploratory-analysis.xlsx", sheetNAME = "sample-info")

################################
# Sheet 2: sample-info-span    #
################################

housing_weighted %>% group_by(HOUSEHOLDSPAN) %>% summarize(n = survey_total()) -> span_household
housing_weighted %>% group_by(HHSPAN2) %>% summarize(n = survey_total()) -> span_HH
...
housing_weighted %>% group_by(HOUSEHOLDRACEETH) %>% summarize(n = survey_total()) -> raceeth_household
housing_weighted %>% group_by(RACEETH) %>% summarize(n = survey_total()) -> raceeth_HH
...
## Racial proportions
house_race <- housing_weighted %>% group_by(HOUSEHOLDRACE) %>% summarize(race_prop = survey_mean())
hh_race <- housing_weighted %>% group_by(HHRACE3) %>% summarize(race_prop = survey_mean())
colnames(house_race)[1] <- colnames(hh_race)[1] <- "Household race"
race_prop <- rbind(house_race[1:5,], hh_race[1:5,]) # COL1

house_span <- housing_weighted %>% group_by(HOUSEHOLDSPAN) %>% summarize(span_prop = survey_mean())
hh_span <- housing_weighted %>% group_by(HHSPAN2) %>% summarize(span_prop = survey_mean())
colnames(house_span)[1] <- colnames(hh_span)[1] <- "Household span"
span_prop <- rbind(house_span[1:2,], hh_span[1:2,]) # COL1

# Geography (Census division)
# by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDRACE, DIVISION) %>% summarize(prop = survey_mean())
...
division.byhouserace <- cbind(tmp2[1:5,c(1,2:10)], tmp3[1:5,c(2:10)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHRACE3, DIVISION) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHRACE3 ~ DIVISION, value.var = "prop")
tmp3 <- dcast(tmp, HHRACE3 ~ DIVISION, value.var = "prop_se")
colnames(tmp3) <- paste(colnames(tmp2), "_se", sep = "")
division.byhhrace <- cbind(tmp2[1:5,c(1,2:10)], tmp3[1:5,c(2:10)])
...
# by spaneth
## by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDSPAN, DIVISION) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HOUSEHOLDSPAN ~ DIVISION, value.var = "prop")
tmp3 <- dcast(tmp, HOUSEHOLDSPAN ~ DIVISION, value.var = "prop_se")
colnames(tmp3) <- paste(colnames(tmp2), "_se", sep = "")
division.byhousespan <- cbind(tmp2[1:2,c(1,2:10)], tmp3[1:2,c(2:10)])
## by #2: householder
tmp <- housing_weighted %>% group_by(HHSPAN2, DIVISION) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHSPAN2 ~ DIVISION, value.var = "prop")
tmp3 <- dcast(tmp, HHSPAN2 ~ DIVISION, value.var = "prop_se")
colnames(tmp3) <- paste(colnames(tmp2), "_se", sep = "")
division.byhhspan <- cbind(tmp2[1:2,c(1,2:10)], tmp3[1:2,c(2:10)])
...
inc.byrace <- rbind(inc.byhouserace[1:5,], inc.byhhrace[1:5,]) # COL3
colnames(inc.byhousespan)[1] <- colnames(inc.byhhspan)[1] <- "Household span"
inc.byspan <- rbind(inc.byhousespan[1:2,], inc.byhhspan[1:2,]) # COL3
...
pov.byrace <- rbind(pov.byhouserace[1:5,], pov.byhhrace[1:5,]) # COL4

colnames(pov.byhousespan)[1] <- colnames(pov.byhhspan)[1] <- "Household span"
pov.byspan <- rbind(pov.byhousespan[1:2,], pov.byhhspan[1:2,]) # COL4

## Tenure
# by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDRACE, TENURE) %>% summarize(prop = survey_mean())
...
tenure.byhouserace <- cbind(tmp2[1:5,c(1,3:4)], tmp3[1:5,c(3:4)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHRACE3, TENURE) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHRACE3 ~ TENURE, value.var = "prop")
colnames(tmp2)[3:4] <- c("Owner", "Renter")
tmp3 <- dcast(tmp, HHRACE3 ~ TENURE, value.var = "prop_se")
colnames(tmp3)[3:4] <- paste(colnames(tmp2)[3:4], "_se", sep = "")
tenure.byhhrace <- cbind(tmp2[1:5,c(1,3:4)], tmp3[1:5,c(3:4)])
...
# by span eth
## by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDSPAN, TENURE) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HOUSEHOLDSPAN ~ TENURE, value.var = "prop")
colnames(tmp2)[3:4] <- c("Owner", "Renter")
tmp3 <- dcast(tmp, HOUSEHOLDSPAN ~ TENURE, value.var = "prop_se")
colnames(tmp3)[3:4] <- paste(colnames(tmp2)[3:4], "_se", sep = "")
tenure.byhousespan <- cbind(tmp2[1:2,c(1,3:4)], tmp3[1:2,c(3:4)])
## by #2: householder
tmp <- housing_weighted %>% group_by(HHSPAN2, TENURE) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHSPAN2 ~ TENURE, value.var = "prop")
colnames(tmp2)[3:4] <- c("Owner", "Renter")
tmp3 <- dcast(tmp, HHSPAN2 ~ TENURE, value.var = "prop_se")
colnames(tmp3)[3:4] <- paste(colnames(tmp2)[3:4], "_se", sep = "")
tenure.byhhspan <- cbind(tmp2[1:2,c(1,3:4)], tmp3[1:2,c(3:4)])
...
## HUDSUB
# by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDRACE, HUDSUB) %>% summarize(prop = survey_mean())
...
hudsub.byhouserace <- cbind(tmp2[1:5,c(1,2:4)], tmp3[1:5,c(2:4)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHRACE3, HUDSUB) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHRACE3 ~ HUDSUB, value.var = "prop")
colnames(tmp2)[2:4] <- c("Other renter", "Public housing", "Voucher recipient")
tmp3 <- dcast(tmp, HHRACE3 ~ HUDSUB, value.var = "prop_se")
colnames(tmp3)[2:4] <- paste(colnames(tmp2)[2:4], "_se", sep = "")
hudsub.byhhrace <- cbind(tmp2[1:5,c(1,2:4)], tmp3[1:5,c(2:4)])
...
# by span eth
## by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDSPAN, HUDSUB) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HOUSEHOLDSPAN ~ HUDSUB, value.var = "prop")
colnames(tmp2)[2:4] <- c("Other renter", "Public housing", "Voucher recipient")
tmp3 <- dcast(tmp, HOUSEHOLDSPAN ~ HUDSUB, value.var = "prop_se")
colnames(tmp3)[2:4] <- paste(colnames(tmp2)[2:4], "_se", sep = "")
hudsub.byhousespan <- cbind(tmp2[1:2,c(1,2:4)], tmp3[1:2,c(2:4)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHSPAN2, HUDSUB) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHSPAN2 ~ HUDSUB, value.var = "prop")
colnames(tmp2)[2:4] <- c("Other renter", "Public housing", "Voucher recipient")
tmp3 <- dcast(tmp, HHSPAN2 ~ HUDSUB, value.var = "prop_se")
colnames(tmp3)[2:4] <- paste(colnames(tmp2)[2:4], "_se", sep = "")
hudsub.byhhspan <- cbind(tmp2[1:2,c(1,2:4)], tmp3[1:2,c(2:4)])
...
# RENTCNTRL
# by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDRACE, RENTCNTRL) %>% summarize(prop = survey_mean())
...
rentcntrl.byhouserace <- cbind(tmp2[1:5,c(1,2:3)], tmp3[1:5,c(2:3)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHRACE3, RENTCNTRL) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHRACE3 ~ RENTCNTRL, value.var = "prop")
colnames(tmp2)[2:3] <- c("No rent control", "Rent control")
tmp3 <- dcast(tmp, HHRACE3 ~ RENTCNTRL, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
rentcntrl.byhhrace <- cbind(tmp2[1:5,c(1,2:3)], tmp3[1:5,c(2:3)])
...
# by span
## by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDSPAN, RENTCNTRL) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HOUSEHOLDSPAN ~ RENTCNTRL, value.var = "prop")
colnames(tmp2)[2:3] <- c("No rent control", "Rent control")
tmp3 <- dcast(tmp, HOUSEHOLDSPAN ~ RENTCNTRL, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
rentcntrl.byhousespan <- cbind(tmp2[1:2,c(1,2:3)], tmp3[1:2,c(2:3)])
## by #2: householder
tmp <- housing_weighted %>% group_by(HHSPAN2, RENTCNTRL) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHSPAN2 ~ RENTCNTRL, value.var = "prop")
colnames(tmp2)[2:3] <- c("No rent control", "Rent control")
tmp3 <- dcast(tmp, HHSPAN2 ~ RENTCNTRL, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
rentcntrl.byhhspan <- cbind(tmp2[1:2,c(1,2:3)], tmp3[1:2,c(2:3)])
...
# DBMISSRENT
# by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDRACE, DBMISSRENT) %>% summarize(prop = survey_mean())
...
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
missrent.byhouserace <- cbind(tmp2[1:5,c(1,2:3)], tmp3[1:5,c(2:3)])
# by #2: householder
tmp <- housing_weighted %>% group_by(HHRACE3, DBMISSRENT) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHRACE3 ~ DBMISSRENT, value.var = "prop")
colnames(tmp2)[2:3] <- c("Not missed rent", "Missed rent")
tmp3 <- dcast(tmp, HHRACE3 ~ DBMISSRENT, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
missrent.byhhrace <- cbind(tmp2[1:5,c(1,2:3)], tmp3[1:5,c(2:3)])
...
# by span
## by #1: household
tmp <- housing_weighted %>% group_by(HOUSEHOLDSPAN, DBMISSRENT) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HOUSEHOLDSPAN ~ DBMISSRENT, value.var = "prop")
colnames(tmp2)[2:3] <- c("Not missed rent", "Missed rent")
tmp3 <- dcast(tmp, HOUSEHOLDSPAN ~ DBMISSRENT, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
missrent.byhousespan <- cbind(tmp2[1:2,c(1,2:3)], tmp3[1:2,c(2:3)])
## by #2: householder
tmp <- housing_weighted %>% group_by(HHSPAN2, DBMISSRENT) %>% summarize(prop = survey_mean())
tmp2 <- dcast(tmp, HHSPAN2 ~ DBMISSRENT, value.var = "prop")
colnames(tmp2)[2:3] <- c("Not missed rent", "Missed rent")
tmp3 <- dcast(tmp, HHSPAN2 ~ DBMISSRENT, value.var = "prop_se")
colnames(tmp3)[2:3] <- paste(colnames(tmp2)[2:3], "_se", sep = "")
missrent.byhhspan <- cbind(tmp2[1:2,c(1,2:3)], tmp3[1:2,c(2:3)])
