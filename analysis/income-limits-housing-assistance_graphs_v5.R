...
housing$HHGRAD6 <- as.factor(housing$HHGRAD6)

# HHCITSHP2
housing <- housing %>% 
  mutate(HHCITSHP2 = ifelse(HHCITSHP3 %in% c("US citizen, native", "US citizen, naturalized"), 
                            "US citizen", 
                            ifelse(HHCITSHP3 == "Not US citizen", "Not US citizen", NA)))
...
housing_weighted %>% filter(HOUSEHOLDRACE != "(Missing)" & 
                     ...
                     FEDSUB3 != "(Missing)" & 
                     !(is.na(FEDSUB3))) %>% 
  group_by(HOUSEHOLDRACE, HHCITSHP, FEDSUB3) %>% 
  summarize(prop = survey_mean(),
            n = survey_total()) -> tmp
tmp <- 
  tmp %>% mutate(CI_lb = prop - tstat * prop_se,
                 CI_ub = prop + tstat * prop_se)
tmp2 <- spread(tmp[c("HOUSEHOLDRACE", "HHCITSHP", "FEDSUB3", "prop")], "FEDSUB3", "prop" )
...
prop.fedsub.byhhsex <- twoway_prop(housing_weighted, "HHSEX",
                                           "FEDSUB5")
...
# info.fedsub.full <- rbind(prop.fedsub.byhouseholdrace,
#                           prop.fedsub.byhouseholdspan[1,])
info.fedsub.byhhsex <- rbind(prop.fedsub.byhhsex[,1:6])

info.fedsub.byhhsex_long <- gather(info.fedsub.byhhsex, key,
                                       estimate, 2:6, factor_key = TRUE)

# setting `key_order`
info.fedsub.byhhsex_long <- 
  info.fedsub.byhhsex_long[order(info.fedsub.byhhsex_long$key,
                                 info.fedsub.byhhsex_long$estimate),]
n.levels <- nlevels(housing$HHSEX) - 1
info.fedsub.byhhsex_long$key_order <- n.levels:1

## SHEET 2: with CIs
fedsub.byhhsex.CI.lb <- 
  prop.fedsub.byhhsex %>% 
  select(group, `Other renter_CI_lb`:`Owner_CI_lb`) %>% 
  gather(., key, CI_lb, `Other renter_CI_lb`:`Owner_CI_lb`, factor_key = TRUE)
levels(fedsub.byhhsex.CI.lb$key) <- substr(levels(fedsub.byhhsex.CI.lb$key), 
                                          1, nchar(levels(fedsub.byhhsex.CI.lb$key))-nchar("_CI_lb"))
fedsub.byhhsex.CI.ub <- 
  prop.fedsub.byhhsex %>% 
  select(group, `Other renter_CI_ub`:`Owner_CI_ub`) %>% 
  gather(., key, CI_ub, `Other renter_CI_ub`:`Owner_CI_ub`, factor_key = TRUE)
levels(fedsub.byhhsex.CI.ub$key) <- substr(levels(fedsub.byhhsex.CI.ub$key), 
                                          1, nchar(levels(fedsub.byhhsex.CI.ub$key))-nchar("_CI_ub"))

info.fedsub.CI <- 
  Reduce(function(x, y) merge(x, y, by = c("group", "key")), 
         list(info.fedsub.byhhsex_long[,1:3], fedsub.byhhsex.CI.lb, fedsub.byhhsex.CI.ub))
info.fedsub.CI.byhhsex <- 
  info.fedsub.CI[order(info.fedsub.CI$key, info.fedsub.CI$estimate),]

# Write to Excel sheet
addWorksheet(wb = excelfile_graph2, sheetName = "fedsub-byhhsex", gridLines = TRUE)
writeData(wb = excelfile_graph2, sheet = "fedsub-byhhsex", 
          x = info.fedsub.byhhsex_long, startCol = 1, startRow = 1)
addWorksheet(wb = excelfile_graph2, sheetName = "CI-fedsub-byhhsex", gridLines = TRUE)
writeData(wb = excelfile_graph2, sheet = "CI-fedsub-byhhsex", 
          x = info.fedsub.CI.byhhsex, startCol = 1, startRow = 1)

# openxlsx::saveWorkbook(excelfile_graph2b, "csv files/graphs_final/graph-2-fedsub-hhsex.xlsx",  overwrite = TRUE)

# (C) BY CITSHP -----------------------------------------------------------

prop.fedsub.byhhcitshp <- twoway_prop(housing_weighted, "HHCITSHP3",
                                      "FEDSUB5")

# combine into one table
# info.fedsub.full <- rbind(prop.fedsub.byhouseholdrace,
#                           prop.fedsub.byhouseholdspan[1,])
info.fedsub.byhhcitshp <- rbind(prop.fedsub.byhhcitshp[,1:6])

info.fedsub.byhhcitshp_long <- gather(info.fedsub.byhhcitshp, key,
                                   estimate, 2:6, factor_key = TRUE)

# setting `key_order`
info.fedsub.byhhcitshp_long <- 
  info.fedsub.byhhcitshp_long[order(info.fedsub.byhhcitshp_long$key,
                                 info.fedsub.byhhcitshp_long$estimate),]
n.levels <- nlevels(housing$HHCITSHP3) - 1
info.fedsub.byhhcitshp_long$key_order <- n.levels:1

## SHEET 2: with CIs
fedsub.byhhcitshp.CI.lb <- 
  prop.fedsub.byhhcitshp %>% 
  select(group, `Other renter_CI_lb`:`Owner_CI_lb`) %>% 
  gather(., key, CI_lb, `Other renter_CI_lb`:`Owner_CI_lb`, factor_key = TRUE)
levels(fedsub.byhhcitshp.CI.lb$key) <- substr(levels(fedsub.byhhcitshp.CI.lb$key), 
                                           1, nchar(levels(fedsub.byhhcitshp.CI.lb$key))-nchar("_CI_lb"))
fedsub.byhhcitshp.CI.ub <- 
  prop.fedsub.byhhcitshp %>% 
  select(group, `Other renter_CI_ub`:`Owner_CI_ub`) %>% 
  gather(., key, CI_ub, `Other renter_CI_ub`:`Owner_CI_ub`, factor_key = TRUE)
levels(fedsub.byhhcitshp.CI.ub$key) <- substr(levels(fedsub.byhhcitshp.CI.ub$key), 
                                           1, nchar(levels(fedsub.byhhcitshp.CI.ub$key))-nchar("_CI_ub"))

info.fedsub.CI <- 
  Reduce(function(x, y) merge(x, y, by = c("group", "key")), 
         list(info.fedsub.byhhcitshp_long[,1:3], fedsub.byhhcitshp.CI.lb, fedsub.byhhcitshp.CI.ub))
info.fedsub.CI.byhhcitshp <- 
  info.fedsub.CI[order(info.fedsub.CI$key, info.fedsub.CI$estimate),]

# Write to Excel sheet
addWorksheet(wb = excelfile_graph2, sheetName = "fedsub-byhhcitshp", gridLines = TRUE)
writeData(wb = excelfile_graph2, sheet = "fedsub-byhhcitshp", 
          x = info.fedsub.byhhcitshp_long, startCol = 1, startRow = 1)
addWorksheet(wb = excelfile_graph2, sheetName = "CI-fedsub-byhhcitshp", gridLines = TRUE)
writeData(wb = excelfile_graph2, sheet = "CI-fedsub-byhhcitshp", 
          x = info.fedsub.CI.byhhcitshp, startCol = 1, startRow = 1)
...
criteria_acs_80 <- "ACSREG_80_FLAG == '1' & !(is.na(ACSREG_80_FLAG))"
criteria_acs_30 <- "ACSREG_30_FLAG == '1' & !(is.na(ACSREG_30_FLAG))"
...
info.acs.CI <- 
  info.acs.CI[order(info.acs.CI$key, info.acs.CI$group),]
...
info.fedsub.subset.acs80_long <- gather(info.fedsub.subset.acs80, key,
                                        estimate_80, 3:5, factor_key = TRUE)
info.fedsub.subset.acs30_long <- gather(info.fedsub.subset.acs30, key,
                                        estimate_30, 3:5, factor_key = TRUE)
...
info.fedsub.subset.acs30_long <- 
  info.fedsub.subset.acs30_long[order(info.fedsub.subset.acs30_long$key,
                                           info.fedsub.subset.acs30_long$estimate),]
info.fedsub.subset.acs30_long$key_order_30 <- n.levels:1

info.fedsub.byracesex.acs <- merge(info.fedsub.subset.acs80_long, 
                                info.fedsub.subset.acs30_long, 
                                by = c("group_1", "group_2", "key"))
...
## including CI
info.acs80.CI.lb <- 
  fedsub.byracesex.acs80 %>% 
  select(group_1, group_2, key, `Housing voucher_CI_lb`:`Public housing_CI_lb`) %>% 
  gather(., key, CI_lb_80, `Housing voucher_CI_lb`:`Public housing_CI_lb`, factor_key = TRUE)
levels(info.acs80.CI.lb$key) <- substr(levels(info.acs80.CI.lb$key), 
                                       1, nchar(levels(info.acs80.CI.lb$key))-nchar("_CI_lb"))
info.acs80.CI.ub <- 
  fedsub.byracesex.acs80 %>% 
  select(group_1, group_2, key, `Housing voucher_CI_ub`:`Public housing_CI_ub`) %>% 
  gather(., key, CI_ub_80, `Housing voucher_CI_ub`:`Public housing_CI_ub`, factor_key = TRUE)
levels(info.acs80.CI.ub$key) <- substr(levels(info.acs80.CI.ub$key), 
                                       1, nchar(levels(info.acs80.CI.ub$key))-nchar("_CI_ub"))

info.acs80.CI <- merge(info.acs80.CI.lb, info.acs80.CI.ub, by = c("group_1", "group_2", "key"))

info.acs30.CI.lb <- 
  fedsub.byracesex.acs30 %>% 
  select(group_1, group_2, key, `Housing voucher_CI_lb`:`Public housing_CI_lb`) %>% 
  gather(., key, CI_lb_30, `Housing voucher_CI_lb`:`Public housing_CI_lb`, factor_key = TRUE)
levels(info.acs30.CI.lb$key) <- substr(levels(info.acs30.CI.lb$key), 
                                       1, nchar(levels(info.acs30.CI.lb$key))-nchar("_CI_lb"))
...
info.acs30.CI <- merge(info.acs30.CI.lb, info.acs30.CI.ub, by = c("group_1", "group_2", "key"))

# merge all
info.acs.CI.byracesex <- 
  Reduce(function(x, y) merge(x, y, by = c("group_1", "group_2", "key")), 
         list(info.fedsub.subset.acs80_long, info.acs80.CI,
              info.fedsub.subset.acs30_long, info.acs30.CI))
info.acs.CI.byracesex <- 
  info.acs.CI.byracesex[order(info.acs.CI.byracesex$key, 
                             info.acs.CI.byracesex$group_1, info.acs.CI.byracesex$group_2),]


# Write to Excel sheet
addWorksheet(wb = excelfile_graph3, sheetName = "fedsub-inc-byrace+sex", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "fedsub-inc-byrace+sex", 
          x = info.fedsub.byracesex.acs, startCol = 1, startRow = 1)
addWorksheet(wb = excelfile_graph3, sheetName = "CI-fedsub-inc-byrace+sex", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "CI-fedsub-inc-byrace+sex", 
          x = info.acs.CI.byracesex, startCol = 1, startRow = 1)
...
info.fedsub.byracecit.acs <- merge(info.fedsub.subset.acs80_long, 
                                   info.fedsub.subset.acs30_long, 
                                   by = c("group_1", "group_2", "key"))
...
# merge all
info.acs.CI.byracecit <- 
  Reduce(function(x, y) merge(x, y, by = c("group_1", "group_2", "key")), 
         list(info.fedsub.subset.acs80_long, info.acs80.CI,
              info.fedsub.subset.acs30_long, info.acs30.CI))
info.acs.CI.byracecit <- 
  info.acs.CI.byracecit[order(info.acs.CI.byracecit$key, 
                              info.acs.CI.byracecit$group_1, info.acs.CI.byracecit$group_2),]


# Write to Excel sheet
addWorksheet(wb = excelfile_graph3, sheetName = "fedsub-inc-byrace+cit", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "fedsub-inc-byrace+cit", 
          x = info.fedsub.byracecit.acs, startCol = 1, startRow = 1)
addWorksheet(wb = excelfile_graph3, sheetName = "CI-fedsub-inc-byrace+cit", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "CI-fedsub-inc-byrace+cit", 
          x = info.acs.CI.byracecit, startCol = 1, startRow = 1)
...
fedsub.byraceeduc.acs80$group_1 <- as.factor(fedsub.byraceeduc.acs80$group_1)
fedsub.byraceeduc.acs80$group_2 <- as.factor(fedsub.byraceeduc.acs80$group_2)
fedsub.byraceeduc.acs30$group_1 <- as.factor(fedsub.byraceeduc.acs30$group_1)
fedsub.byraceeduc.acs30$group_2 <- as.factor(fedsub.byraceeduc.acs30$group_2)
...
info.fedsub.byraceeduc.acs <- merge(info.fedsub.subset.acs80_long, 
                                   info.fedsub.subset.acs30_long, 
                                   by = c("group_1", "group_2", "key"))
...
# merge all
info.acs.CI.byraceeduc <- 
  Reduce(function(x, y) merge(x, y, by = c("group_1", "group_2", "key")), 
         list(info.fedsub.subset.acs80_long, info.acs80.CI,
              info.fedsub.subset.acs30_long, info.acs30.CI))
info.acs.CI.byraceeduc <- 
  info.acs.CI.byraceeduc[order(info.acs.CI.byraceeduc$key, 
                              info.acs.CI.byraceeduc$group_1, info.acs.CI.byraceeduc$group_2),]


# Write to Excel sheet
addWorksheet(wb = excelfile_graph3, sheetName = "fedsub-inc-byrace+educ", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "fedsub-inc-byrace+educ", 
          x = info.fedsub.byraceeduc.acs, startCol = 1, startRow = 1)
addWorksheet(wb = excelfile_graph3, sheetName = "CI-fedsub-inc-byrace+educ", gridLines = TRUE)
writeData(wb = excelfile_graph3, sheet = "CI-fedsub-inc-byrace+educ", 
          x = info.acs.CI.byraceeduc, startCol = 1, startRow = 1)

# Write sheets to Excel workbook
openxlsx::saveWorkbook(excelfile_graph3, "csv files/graphs_final/graph-3-reg.xlsx",  overwrite = TRUE)
