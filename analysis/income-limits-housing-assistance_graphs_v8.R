...
twoway_table <- function(group_var, sub_var) {
  prop.full <- twoway_prop(df, group_var, sub_var)
  nSub <- nlevels(housing[[sub_var]]) - 1
  prop.sub <- prop.full[,1:(nSub+1)]
  
  if(sub_var %in% race.defs) {
    index <- which(sub_var %in% race.defs)
    prop.span <- twoway_prop(df, group_var, span.defs[index])
...
# (C) by CITSHP:
twoway_fedsub_table("HOUSEHOLDCITSHP", "FEDSUB4")
twoway_table("FEDSUB4", "HOUSEHOLDCITSHP")
