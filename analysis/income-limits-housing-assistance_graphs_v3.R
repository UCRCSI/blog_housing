...
twoway_prop <- function(df, group_var1, group_var2) {
  ...
  colnames(tmp4) <- paste(colnames(tmp2), "_n", sep = "")
  n.col <- ncol(tmp2)
  final <- cbind(tmp2, tmp3[,c(2:n.col)], tmp4[,c(2:ncol(tmp4))])
  return (final)  
}
...
info.acs.byhousehold_long <- rbind(prop.acs80.byhouseholdrace,
                                   prop.acs80.byhouseholdspan,
                                   prop.acs50.byhouseholdrace,
                                   prop.acs50.byhouseholdspan,
...
info.acs.byhousehold_long <- gather(info.acs.byhousehold, key, estimate, `<30%`:`50-80%`)
## setting `key_order`
